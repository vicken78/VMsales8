...
namespace VMsales8.ViewModels
{
    ...
    {
        public bool isproductinventory;
        ...
        public async Task SaveCommand()
        {
            ...
            ObservableCollection<PurchaseOrderModel> differences = dataProcessor.Compare(ObservableCollectionPurchaseOrderModelDirty, ObservableCollectionPurchaseOrderModelClean);
                ...
                {
                    {
                        switch (item.Action)
                        {
                            case "Update":
                                ...
                                Debug.WriteLine("Difference found:");

                                // Log primary key
                                Debug.WriteLine($"purchase_order_pk: {item.purchase_order_pk}");
                                Debug.WriteLine($"purchase_order_detail_pk: {item.purchase_order_detail_pk}");

                                // Log other relevant properties (you can add/remove properties based on what you want to debug)
                                Debug.WriteLine($"supplier_fk: {item.supplier_fk}");
                                Debug.WriteLine($"purchase_date: {item.purchase_date}");
                                Debug.WriteLine($"invoice_number: {item.invoice_number}");
                                Debug.WriteLine($"lot_name: {item.lot_name}");
                                Debug.WriteLine($"lot_number: {item.lot_number}");
                                Debug.WriteLine($"lot_cost: {item.lot_cost}");
                                Debug.WriteLine($"lot_quantity: {item.lot_quantity}");
                                Debug.WriteLine($"lot_description: {item.lot_description}");
                                Debug.WriteLine($"isproductinventory: {item.isproductinventory}");
                                Debug.WriteLine("--------------------------------------");

                                // Log action to see if it's 'Insert', 'Update', or 'Delete'
                                Debug.WriteLine($"Action: {item.Action}");
        ...
        public void initial_load()
        {
            ...
            ObservableCollectionPurchaseOrderModelClean = new ObservableCollection<PurchaseOrderModel>(ObservableCollectionPurchaseOrderModelDirty.Select(item => new PurchaseOrderModel
            {
                ...
                isproductinventory = item.isproductinventory
